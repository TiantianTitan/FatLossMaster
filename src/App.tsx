...
export default function App() {
  ...
  const bodyDefaults:BodyProfile={heightCm:rawToday?.heightCm??descending.find(r=>r.heightCm!=null)?.heightCm,weightKg:rawToday?.weightKg??descending.find(r=>r.weightKg!=null)?.weightKg,waistCm:rawToday?.waistCm??descending.find(r=>r.waistCm!=null)?.waistCm}
  const dailyDefaults:Partial<DailyRecord>={...defaultsForDate(records,todayKey()),heightCm:bodyDefaults.heightCm,weightKg:bodyDefaults.weightKg}
  const updateTodayBody=async(body:BodyProfile)=>{const now=new Date().toISOString();const resting=rawToday?.restingMode==='manual'&&rawToday.restingCalories?rawToday.restingCalories:estimateRestingCalories(body.weightKg)??DEFAULT_RESTING_CALORIES;const activityLevel=rawToday?.activityLevel??DEFAULT_ACTIVITY_LEVEL;const next:DailyRecord={...(rawToday??{...dailyDefaults,id:crypto.randomUUID(),date:todayKey(),createdAt:now,updatedAt:now}),...body,restingCalories:resting,restingMode:rawToday?.restingMode??'auto',activityLevel,dailyCalories:estimateDailyActivityCalories(resting,activityLevel),updatedAt:now};await saveRecord(next);reload()}
