...
import { calculateCalorieDeficit, calculateTotalCalories } from './calculations'
...
export const exportCSV = (records: DailyRecord[]) => {
  ...
  const rows = records.map(r => ({ ...r, totalCalories: calculateTotalCalories(r), calorieDeficit: calculateCalorieDeficit(r) }))
...
export const exportJSON = (records: DailyRecord[]) => download(JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), records }, null, 2), 'application/json', `轻衡备份-${new Date().toISOString().slice(0,10)}.json`)
