...
export function RecordForm({ record, onUpdate }: { record: DailyRecord; onUpdate: (patch: Partial<DailyRecord>) => void }) {
  ...
  const resetResting = () => { const resting = estimateRestingCalories(record.weightKg) ?? DEFAULT_RESTING_CALORIES; onUpdate({ restingMode: 'auto', restingCalories: resting, dailyCalories: estimateDailyActivityCalories(resting, record.activityLevel ?? 'sedentary') }) }
  return <div className="form-stack">
    ...
    <section className="form-section"><div className="form-title-row"><h2>静息消耗</h2><button type="button" onClick={resetResting}><RotateCcw size={14}/>恢复估算</button></div><div className="input-card"><label className="input-row"><span>全天静卧消耗</span><span className="input-wrap"><DecimalInput commitOnBlur value={record.restingCalories} onValueChange={value=>{const resting=value&&value>0?value:estimateRestingCalories(record.weightKg)??DEFAULT_RESTING_CALORIES;onUpdate({restingCalories:resting,restingMode:value&&value>0?'manual':'auto',dailyCalories:estimateDailyActivityCalories(resting,record.activityLevel??'sedentary')})}} placeholder="自动估算"/><small>kcal</small></span></label></div><p className="field-help">默认继承最近一次体重，按 22 kcal × 体重估算；尚无体重时暂用 1,600 kcal，可手动修改。</p></section>
