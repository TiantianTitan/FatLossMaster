...
const bodyFields: Array<{key:NumericRecordKey;label:string;unit:string;placeholder:string}> = [{key:'waistCm',label:'腰围',unit:'cm',placeholder:'84,5'}]
...
export function RecordForm({ record, onUpdate }: { record: DailyRecord; onUpdate: (patch: Partial<DailyRecord>) => void }) {
  ...
  return <div className="form-stack">
    <section className="form-section"><h2>当日测量</h2><div className="profile-reference"><span>首页身体数据</span><strong>{record.heightCm?.toLocaleString()??'—'} cm · {record.weightKg?.toLocaleString()??'—'} kg</strong></div><div className="input-card">{bodyFields.map(field => <label className="input-row" key={field.key}><span>{field.label}</span><span className="input-wrap"><DecimalInput value={record[field.key]} onValueChange={value=>updateBody(field.key,value)} placeholder={field.placeholder}/><small>{field.unit}</small></span></label>)}</div><div className="inline-result"><span>根据首页数据计算 BMI</span><strong>{bmi?.toFixed(1) ?? '—'}</strong></div></section>

    <section className="form-section"><div className="form-title-row"><h2>基础静息消耗</h2><button onClick={resetResting}><RotateCcw size={14}/>恢复估算</button></div><div className="input-card"><label className="input-row"><span>全天静卧消耗</span><span className="input-wrap"><DecimalInput value={record.restingCalories} onValueChange={resting=>onUpdate({restingCalories:resting,restingMode:'manual',dailyCalories:estimateDailyActivityCalories(resting,record.activityLevel)})} placeholder="输入体重后估算"/><small>kcal</small></span></label></div><p className="field-help">相当于全天躺着不动、维持身体基本运转的消耗。默认按 22 kcal × 当前体重估算，可手动修改。</p></section>
