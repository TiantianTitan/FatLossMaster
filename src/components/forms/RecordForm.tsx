...
export function RecordForm({ record, onUpdate }: { record: DailyRecord; onUpdate: (patch: Partial<DailyRecord>) => void }) {
  ...
  return <div className="form-stack">
    ...
    <section className="form-section"><h2>日常消耗</h2><div className="activity-levels">{activityLevels.map(level => <button key={level.id} aria-pressed={record.activityLevel===level.id} className={record.activityLevel===level.id?'active':''} onClick={() => setActivityLevel(level.id)}><span><strong>{level.name}</strong><small>{level.description}</small></span><em>+{estimateDailyActivityCalories(record.restingCalories,level.id)?.toLocaleString() ?? '—'} kcal</em></button>)}</div><div className="input-card steps-card"><label className="input-row"><span>今日步数 <small>可选</small></span><span className="input-wrap"><DecimalInput commitOnBlur value={record.stepCount} onValueChange={stepCount=>onUpdate({stepCount:stepCount == null ? undefined : Math.max(0,stepCount)})} placeholder="未记录"/><small>步</small></span></label></div><div className="inline-result"><span>今日日常消耗</span><strong>+{adjustedDaily?.toLocaleString() ?? '—'} kcal</strong></div></section>

    <section className="form-section"><h2>睡眠时间</h2><div className="input-card"><label className="input-row"><span>昨晚睡眠</span><span className="input-wrap"><DecimalInput commitOnBlur value={record.sleepHours} onValueChange={sleepHours=>onUpdate({sleepHours:sleepHours == null ? undefined : Math.min(sleepHours,24)})} placeholder="例如 7,5"/><small>h</small></span></label></div></section>

    <section className="form-section"><h2>备注</h2><DraftTextarea className="notes" rows={4} placeholder="今天感觉如何？" value={record.notes} onCommit={notes=>onUpdate({notes})}/></section>
  </div>
}
