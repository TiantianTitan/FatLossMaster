...
export function useDailyRecord(date: string, onSaved: () => void, defaults?: Partial<DailyRecord>) {
  ...
  useEffect(() => { let active = true; const dailyDefaults={heightCm:defaultHeight,weightKg:defaultWeight,restingCalories:defaultResting,activityLevel:defaultLevel,dailyCalories:defaultDaily,restingMode:'auto' as const}; getRecordByDate(date).then(found => { if (active) { const validResting=found?.restingCalories!=null&&found.restingCalories>0,validDaily=found?.dailyCalories!=null&&found.dailyCalories>0;setRecord(found ? { ...found, heightCm: found.heightCm ?? defaultHeight, weightKg: found.weightKg ?? defaultWeight, restingCalories:validResting?found.restingCalories:defaultResting,restingMode:validResting?found.restingMode??'manual':'auto',activityLevel:found.activityLevel??defaultLevel,dailyCalories:validDaily?found.dailyCalories:defaultDaily } : emptyRecord(date, dailyDefaults)); setLoadedDate(date) } }); return () => { active = false } }, [date, defaultHeight, defaultWeight,defaultResting,defaultLevel,defaultDaily])
