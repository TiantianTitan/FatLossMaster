import { ArrowUpRight, Dumbbell, Footprints, Gauge, Moon, Pencil, Plus, Utensils } from 'lucide-react'
...
export function TodayPage({ record, bodyDefaults, onQuickAddFood, onQuickAddActivity, onEdit }: { record?: DailyRecord; bodyDefaults: BodyProfile; onQuickAddFood: (entry:FoodEntry)=>Promise<void>; onQuickAddActivity:(entry:ActivityEntry)=>Promise<void>; onEdit: () => void }) {
  ...
  return <main className="page today-page">
    ...
    <section className="metric-grid">
      ...
      <article className="metric-card compact"><span className="metric-symbol"><Dumbbell size={17}/></span><span>运动</span><Value value={getExerciseCalories(record)} unit="kcal"/></article>
      <article className="metric-card compact sleep-card"><span className="metric-symbol"><Moon size={17}/></span><span>睡眠</span><Value value={record?.sleepHours} unit="h"/></article>
      <article className="metric-card compact"><span className="metric-symbol"><Footprints size={17}/></span><span>步数</span><Value value={record?.stepCount} unit="步"/></article>
      <article className="metric-card compact"><span className="metric-symbol"><Gauge size={17}/></span><span>日常消耗</span><Value value={getDailyActivityCalories(record)} unit="kcal"/></article>
    </section>
