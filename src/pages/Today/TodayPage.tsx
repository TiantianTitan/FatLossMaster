...
export function TodayPage({ record, onEdit }: { record?: DailyRecord; onEdit: () => void }) {
  ...
  return <main className="page today-page">
    ...
    <section className={`hero-card ${deficit != null && deficit < 0 ? 'surplus' : ''}`}>
      ...
      <div className="energy-row"><div><span>摄入</span><Value value={record?.foodCalories} unit="kcal"/></div><ArrowUpRight size={20}/><div><span>消耗</span><Value value={total || undefined} unit="kcal"/></div></div>
    </section>
    <section className="metric-grid">
      <article className="metric-card accent"><span>蛋白质</span><Value value={record?.proteinGrams} unit="g"/><i>PROTEIN</i></article>
      ...
      <article className="metric-card wide"><div className="metric-icon"><Dumbbell size={20}/></div><div><span>运动消耗</span><Value value={record?.exerciseCalories} unit="kcal"/></div><div className="mini-bars"><i/><i/><i/><i/><i/></div></article>
    </section>
