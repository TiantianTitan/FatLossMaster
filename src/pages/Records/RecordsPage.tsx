...
export function RecordsPage({ initialDate, records, onRecordsChange }: { initialDate: string; records: DailyRecord[]; onRecordsChange: () => void }) {
  ...
  return <main className="page records-page">
    ...
    <div className="date-stepper compact"><button className="icon-button" aria-label="前一天" onClick={()=>selectDate(addDays(parseISO(selected),-1))}><ChevronLeft/></button><button className="date-picker-button" aria-expanded={calendarOpen} onClick={()=>setCalendarOpen(open=>!open)}><span>{selected===todayKey()?'正在记录今天':'正在查看'}</span><strong>{fullDisplayDate(selected)}</strong><ChevronDown size={15} className={calendarOpen?'open':''}/></button><button className="icon-button" aria-label="后一天" onClick={()=>selectDate(addDays(parseISO(selected),1))}><ChevronRight/></button></div>
    {calendarOpen&&<section className="calendar-card collapsible"><div className="calendar-head"><button className="icon-button" aria-label="上个月" onClick={() => setMonth(addMonths(month,-1))}><ChevronLeft/></button><strong>{format(month,'yyyy年 M月')}</strong><button className="icon-button" aria-label="下个月" onClick={() => setMonth(addMonths(month,1))}><ChevronRight/></button></div><div className="calendar-grid">{weekdays.map(d=><span className="weekday" key={d}>{d}</span>)}{days.map(day => { const key=format(day,'yyyy-MM-dd'); return <button key={key} aria-label={fullDisplayDate(key)} aria-current={key===selected?'date':undefined} className={`${key===selected?'selected ':''}${!isSameMonth(day,month)?'muted ':''}${marked.has(key)?'marked':''}`} onClick={()=>selectDate(day,true)}><span>{day.getDate()}</span></button> })}</div></section>}
