...
export function RecordsPage({ initialDate, records, dailyDefaults, onRecordsChange }: { initialDate: string; records: DailyRecord[]; dailyDefaults: Partial<DailyRecord>; onRecordsChange: () => void }) {
  ...
  const { record, update, ready, saved } = useDailyRecord(selected, onRecordsChange, selected===todayKey()?dailyDefaults:undefined)
  ...
  return <main className="page records-page">
    ...
    {!calendarOpen&&<button className="calendar-shortcut" onClick={()=>setCalendarOpen(true)}><CalendarDays size={16}/>选择其他日期<span>{marked.size} 天已有记录</span></button>}
