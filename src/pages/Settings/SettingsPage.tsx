...
export function SettingsPage({records,bodyDefaults,onUpdateBody,theme,setTheme,onRecordsChange}:{records:DailyRecord[];bodyDefaults:BodyProfile;onUpdateBody:(body:BodyProfile)=>Promise<void>;theme:ThemeMode;setTheme:(t:ThemeMode)=>void;onRecordsChange:()=>void}) {
 ...
 return <main className="page settings-page"><header className="page-header"><div><p className="eyebrow">本机与隐私</p><h1>设置</h1></div></header>
  <section className="settings-section"><h2>身体资料</h2><button className="measurement-card" onClick={()=>setEditingBody(true)}><span className="measurement-symbol"><Ruler size={22}/></span><span><small>Mifflin–St Jeor 计算资料</small><strong>{bodyDefaults.weightKg?.toLocaleString()??'—'} kg</strong><em>{bodyDefaults.sex==='male'?'男':bodyDefaults.sex==='female'?'女':'性别未填'} · {bodyDefaults.ageYears?.toLocaleString()??'—'} 岁 · {bodyDefaults.heightCm?.toLocaleString()??'—'} cm · BMI {calculateBMI(bodyDefaults.weightKg,bodyDefaults.heightCm)?.toFixed(1)??'—'}</em></span><Pencil size={17}/></button></section>
  ...
  {editingBody&&<BodyEditor value={bodyDefaults} onClose={()=>setEditingBody(false)} onSave={async body=>{await onUpdateBody(body);setEditingBody(false);show('身体资料已保存')}}/>}{pending&&<Modal title="恢复数据" onClose={()=>setPending(null)}><div className="restore-copy"><strong>已验证 {pending.length} 条记录</strong><p>“合并”会按日期写入备份，保留其他日期；“覆盖”会先清空现有数据。请选择恢复方式。</p></div><div className="modal-actions"><button className="secondary-button" onClick={()=>restore('merge')}>合并数据</button><button className="danger-fill" onClick={()=>restore('replace')}>覆盖数据</button></div></Modal>}{toast&&<Toast>{toast}</Toast>}
 </main>
}
