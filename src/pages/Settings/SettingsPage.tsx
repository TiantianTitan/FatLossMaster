import { ChevronRight, Clock3, Database, Download, FileJson, Info, LockKeyhole, Moon, Pencil, RotateCcw, Ruler, SunMedium, Trash2 } from 'lucide-react'
...
const LAST_BACKUP_KEY='light-balance:last-json-export'
const backupLabel=(value:string)=>{if(!value)return'尚未导出';const date=new Date(value);if(Number.isNaN(date.getTime()))return'尚未导出';const today=new Date();return date.toDateString()===today.toDateString()?`今天 ${date.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit'})}`:date.toLocaleDateString('zh-CN',{year:'numeric',month:'numeric',day:'numeric'})}
const backupIsOverdue=(value:string,hasRecords:boolean)=>{if(!hasRecords)return false;const timestamp=new Date(value).getTime();return !Number.isFinite(timestamp)||Date.now()-timestamp>30*24*60*60*1000}
...
export function SettingsPage({records,bodyDefaults,onUpdateBody,theme,setTheme,onRecordsChange}:{records:DailyRecord[];bodyDefaults:BodyProfile;onUpdateBody:(body:BodyProfile)=>Promise<void>;theme:ThemeMode;setTheme:(t:ThemeMode)=>void;onRecordsChange:()=>void}) {
 const fileRef=useRef<HTMLInputElement>(null),[pending,setPending]=useState<DailyRecord[]|null>(null),[toast,setToast]=useState(''),[editingBody,setEditingBody]=useState(false),[lastBackupAt,setLastBackupAt]=useState(()=>{try{return localStorage.getItem(LAST_BACKUP_KEY)??''}catch{return''}})
 ...
 const restore=async(mode:'merge'|'replace')=>{if(!pending)return;try{if(mode==='merge')await mergeRecords(pending);else await replaceRecords(pending);setPending(null);onRecordsChange();show(`已恢复 ${pending.length} 条记录`)}catch{show('恢复失败，请重试')}}
 ...
 const backup=()=>{exportJSON(records);const timestamp=new Date().toISOString();try{localStorage.setItem(LAST_BACKUP_KEY,timestamp)}catch{void 0}setLastBackupAt(timestamp);show('JSON 备份已导出')}
 const backupOverdue=backupIsOverdue(lastBackupAt,records.length>0)
