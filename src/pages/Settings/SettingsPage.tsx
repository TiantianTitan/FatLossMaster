import { ChevronRight, Database, Download, FileJson, Info, LockKeyhole, Moon, RotateCcw, SunMedium, Trash2 } from 'lucide-react'
...
export function SettingsPage({records,theme,setTheme,onRecordsChange}:{records:DailyRecord[];theme:ThemeMode;setTheme:(t:ThemeMode)=>void;onRecordsChange:()=>void}) {
 const fileRef=useRef<HTMLInputElement>(null),[pending,setPending]=useState<DailyRecord[]|null>(null),[toast,setToast]=useState('')
 ...
 return <main className="page settings-page"><header className="page-header"><div><p className="eyebrow">本机与隐私</p><h1>设置</h1></div></header>
  ...
  {pending&&<Modal title="恢复数据" onClose={()=>setPending(null)}><div className="restore-copy"><strong>已验证 {pending.length} 条记录</strong><p>“合并”会按日期写入备份，保留其他日期；“覆盖”会先清空现有数据。请选择恢复方式。</p></div><div className="modal-actions"><button className="secondary-button" onClick={()=>restore('merge')}>合并数据</button><button className="danger-fill" onClick={()=>restore('replace')}>覆盖数据</button></div></Modal>}{toast&&<Toast>{toast}</Toast>}
 </main>
}
